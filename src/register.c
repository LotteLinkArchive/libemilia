...
hh_status_t hh_mkregister(struct hh_register_s *target, bool autosort, bool cuckoo, hh_mt19937_ro_t *random)
{
        ...
        struct hh_register_s out = {.elements   = __hh_dyn_mk(struct hh_register_el_s),
                                    .sorted     = true,
                                    .sorting    = autosort,
        ...
        if (out.en_cuckoo)
                ccrt = hh_cuckoo_filter_new(&out.cuckoo, INT16_MAX, INT8_MAX, (uint32_t)(out.identifier & 0xffffffff));
        else
                ccrt = HH_CUCKOO_FILTER_OK;
...
int hh_register_geti(struct hh_register_s *reg, uint64_t id)
{
        ...
        if (reg->sorted) return hh_i_bsr_reg(reg->elements, hh_register_els(reg), id);
...
hh_status_t hh_register_add(struct hh_register_s *reg, uint64_t id, void *data)
{
        ...
        struct hh_register_el_s temp_el = {.data = data, .identifier = id};
...
unsigned int hh_register_els(struct hh_register_s *reg) { return __hh_dyn_count(reg->elements); }
